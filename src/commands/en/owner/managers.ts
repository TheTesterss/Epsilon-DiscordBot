...
export = {
    ...
    options: [
        {
            ...
            options: [
                ...
                {
                    name: 'premium',
                    nameLocalizations: { fr: 'premium' },
                    description: 'Used to manage the premium users.',
                    descriptionLocalizations: {
                        fr: 'Utilisé pour gérer les utilisateurs premium.'
                    },
                    type: ApplicationCommandOptionType.Subcommand
                }
            ]
    ...
    en: async (
        ...
        const rows = [];
        if (myRow1.components.length > 0) rows.push(myRow1);
        if (myRow2.components.length > 0) rows.push(myRow2);
                ...
                if (i.isUserSelectMenu() || i.isStringSelectMenu()) {
                    ...
                    for (const value of i.values) {
                        ...
                        if (user) {
                            if (sub === 'black') {
                                user.blacklist = {
                                    isBlacklisted: result ? true : false,
                            ...
                            if (sub === 'white') {
                                user.whitelist = {
                                    isWhitelisted: result ? true : false,
                                    by: result ? i.user.id : undefined,
                                    dates: {
                                        from: result ? new Date() : undefined
                                    }
                    ...
                    action = {
                        ...
                        actionString: result
                            ? `${i.user.username} just added ${i.client.users.cache.get(i.values[0])!.username} and ${i.values.length} others to the blacklist.`
                            : `${i.user.username} just removed ${i.client.users.cache.get(i.values[0])!.username} and  and ${i.values.length} from the whitelist.`
                    };
                    ...
                    await message.edit({
                        ...
                        components: [myUpdatedRow1, myUpdatedRow2]
                    });
                }
                ...
                if (i.isButton()) {
                    ...
                    if (['righta', 'right', 'left', 'lefta', 'back'].includes(ids[2])) {
                        ...
                        await message.edit({
                            ...
                            components: [
                                new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(components[0]),
                                new ActionRowBuilder<MessageActionRowComponentBuilder>().addComponents(components[1])
                            ]
                        });
                    }
                }
